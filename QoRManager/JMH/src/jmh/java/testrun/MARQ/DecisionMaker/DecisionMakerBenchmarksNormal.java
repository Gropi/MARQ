...
@Fork(value = 4)
public class DecisionMakerBenchmarksNormal {
    private static final Logger _Logger = LogManager.getLogger("executionLog");

    @State(Scope.Benchmark)
    public static class GraphState {
        private IGraph m_RandomizedGraphNormal;
    ...
    public void MobiDicAverage(GraphState state, Blackhole blackhole) {
        var instanceUnderTest = new MobiDiCManager(state.m_RandomizedGraphNormal);
    ...
    public void EdgeDicEConstraintThroughput(GraphState state, Blackhole blackhole) {
        ...
        var instanceUnderTest = new EdgeDiCManager(state.m_RandomizedGraphNormal, m_Conditions, constraints, null, m_NormalizingMode, m_MostImportantCriteria, _Logger, "EConstraint");
        blackhole.consume(instanceUnderTest.getInitialSelection(Integer.MAX_VALUE, null));
    }
    ...
    public void EdgeDicTopsisThroughput(GraphState state, Blackhole blackhole) {
        ...
        var instanceUnderTest = new EdgeDiCManager(state.m_RandomizedGraphNormal, m_Conditions, constraints, m_Weights, m_NormalizingMode, m_MostImportantCriteria, _Logger, "topsis");
        blackhole.consume(instanceUnderTest.getInitialSelection(Integer.MAX_VALUE, null));
    }
